package comandos

import (
	strct "File_Manager_GO/structs"
	"bytes"
	"encoding/binary"
	"os"
	"strconv"
	"strings"
	"time"
	"unsafe"
)

type Sesion struct {
	Id_user     int
	Id_grp      int
	Start_SB    int
	System_type int
	User_name   string
	Path        string
	Fit         [1]byte
}

var CurrentSession Sesion
var IsLoginFlag bool = false

func EjecutarLOGIN(user_name string, user_password string, id_mount string) {
	// Se limpia la cadena Consola para recolectar informacion de una nueva ejecucion
	Consola = ""

	error_ := 0
	pathDisco_Partition := ""
	startPartition := 0
	// sizePartition no se utiliza, se creo solo por la referencia en getDatosID
	sizePartition := 0

	// NOTA: LA EXTRACCION DE DATOS SE IMPLEMENTO SOLO PARA PARTICIONES PRIMARIAS
	getDatosID(id_mount, &pathDisco_Partition, &startPartition, &sizePartition, &error_)

	// Si 'error_' obtiene un 1, el path del ID montado no existe
	if error_ == 1 {
		return
	}

	// Apertura del archivo del disco binario
	disco_actual, err := os.OpenFile(pathDisco_Partition, os.O_RDWR, 0660)
	if err != nil {
		msg_error(err)
		return
	}
	defer disco_actual.Close()

	// Estructuras necesarias a utilizar
	superB := strct.SuperBloque{}
	inodo := strct.InodoTable{}
	inodoU := strct.InodoTable{}

	// Tamaño de algunas estructuras
	var inodoTable strct.InodoTable
	const i_size = unsafe.Sizeof(inodoTable)

	// --------Se extrae el SB del disco---------
	var sbsize int = int(binary.Size(superB))
	disco_actual.Seek(int64(startPartition), 0)
	data := leerEnFILE(disco_actual, sbsize)
	buffer := bytes.NewBuffer(data)
	err = binary.Read(buffer, binary.BigEndian, &superB)
	if err != nil {
		Consola += "Binary.Read failed\n"
		msg_error(err)
	}

	// --------Se extrae el InodoTable del archivo user.txt---------
	var inodosize int = int(binary.Size(inodo))
	disco_actual.Seek(int64(byteToInt(superB.S_inode_start[:])+int(i_size)), 0)
	dataI := leerEnFILE(disco_actual, inodosize)
	bufferI := bytes.NewBuffer(dataI)
	err = binary.Read(bufferI, binary.BigEndian, &inodo)
	if err != nil {
		Consola += "Binary.Read failed\n"
		msg_error(err)
	}

	// Actualizacion de ultima fecha que se visito el archivo sin modificarlo
	copy(inodo.I_atime[:], time.Now().String())

	// Se guarda el struct del inodo del archivo users.txt
	// Al inode_start se le suma el tamaño de otro inodo struct, pues en el start esta el struct de Carpeta root
	disco_actual.Seek(int64(byteToInt(superB.S_inode_start[:])+int(i_size)), 0)
	sInodoA := &inodo
	var binario1 bytes.Buffer
	binary.Write(&binario1, binary.BigEndian, sInodoA)
	escribirDentroFILE(disco_actual, binario1.Bytes())

	// Actualizacion de los primeros datos para indicar una Sesion Activa
	CurrentSession.Start_SB = startPartition
	copy(CurrentSession.Fit[:], "W")
	CurrentSession.System_type = byteToInt(superB.S_filesystem_type[:])

	// VERIFICACION DE EXISTENCIA DE DATOS EN EL ARCHIVO USERS.TXT

	// Almacenara lo extraido del archivo
	contenidoFile := ""
	// --------Se extrae el InodoTable del archivo user.txt nuevamente---------
	var inodoUsize int = int(binary.Size(inodoU))
	disco_actual.Seek(int64(byteToInt(superB.S_inode_start[:])+int(i_size)), 0)
	dataIU := leerEnFILE(disco_actual, inodoUsize)
	bufferIU := bytes.NewBuffer(dataIU)
	err = binary.Read(bufferIU, binary.BigEndian, &inodoU)
	if err != nil {
		Consola += "Binary.Read failed\n"
		msg_error(err)
	}

	for i := 0; i < 15; i++ {

		// El 255 representa al -1
		if int(inodoU.I_block[i]) != 255 {
			archivo := strct.BloqueArchivo{}
			disco_actual.Seek(int64(byteToInt(superB.S_block_start[:])), 0)

			for j := 0; j <= int(inodoU.I_block[i]); j++ {
				// --------Se extrae el Bloque Archivo USERS.TXT---------
				var basize int = int(binary.Size(archivo))
				data := leerEnFILE(disco_actual, basize)
				buff := bytes.NewBuffer(data)
				err = binary.Read(buff, binary.BigEndian, &archivo)
				if err != nil {
					Consola += "Binary.Read failed\n"
					msg_error(err)
				}
			}

			//Concatenar el contenido de cada bloque perteneciente al archivo users.txt
			contenidoFile += byteToStr(archivo.B_content[:])
		}
	}

	disco_actual.Close()

	var arregloU_G []string = strings.Split(contenidoFile, "\n")

	for _, filaU_G := range arregloU_G {

		// Se verifica que la fila obtenida del contenido no venga vacia
		if filaU_G != "" {
			var data []string = strings.Split(filaU_G, ",")

			// Verificar ID que no se un U/G eliminado
			if strings.Compare(data[0], "0") != 0 {

				// Verificar que sea tipo Usuario
				if strings.Compare(data[1], "U") == 0 {
					group := data[2]
					user_ := data[3]
					password := data[4]
					if strings.Compare(user_, user_name) == 0 {
						if strings.Compare(password, user_password) == 0 {
							CurrentSession.Path = pathDisco_Partition
							CurrentSession.User_name = user_
							CurrentSession.Id_user, _ = strconv.Atoi(data[0])
							CurrentSession.Id_grp = buscarGrupo(group)

							Consola += "Sesion iniciada con exito!\n"
							IsLoginFlag = true
							return
						} else {
							Consola += "Error: Contraseña incorrecta\n"
							return
						}
					}

				}
			}
		}
	}

	Consola += "Error: Usuario no encontrado!\n"
}

func buscarGrupo(grp_name string) int {

	// Apertura del archivo del disco binario
	disco_actual, err := os.OpenFile(CurrentSession.Path, os.O_RDWR, 0660)
	if err != nil {
		msg_error(err)
		return -1
	}
	defer disco_actual.Close()

	// Estructuras necesarias a utilizar
	superB := strct.SuperBloque{}
	inodo := strct.InodoTable{}

	// Tamaño de algunas estructuras
	var inodoTable strct.InodoTable
	const i_size = unsafe.Sizeof(inodoTable)

	// --------Se extrae el SB del disco---------
	var sbsize int = int(binary.Size(superB))
	disco_actual.Seek(int64(CurrentSession.Start_SB), 0)
	data := leerEnFILE(disco_actual, sbsize)
	buffer := bytes.NewBuffer(data)
	err = binary.Read(buffer, binary.BigEndian, &superB)
	if err != nil {
		Consola += "Binary.Read failed\n"
		msg_error(err)
	}

	// --------Se extrae el InodoTable del archivo user.txt---------
	var inodosize int = int(binary.Size(inodo))
	disco_actual.Seek(int64(byteToInt(superB.S_inode_start[:])+int(i_size)), 0)
	dataI := leerEnFILE(disco_actual, inodosize)
	bufferI := bytes.NewBuffer(dataI)
	err = binary.Read(bufferI, binary.BigEndian, &inodo)
	if err != nil {
		Consola += "Binary.Read failed\n"
		msg_error(err)
	}

	// Almacenara lo extraido del archivo
	contenidoFile := ""

	// Se recorren los iblock para conocer los punteros
	for i := 0; i < 15; i++ {

		// El 255 representa al -1
		if int(inodo.I_block[i]) != 255 {
			archivo := strct.BloqueArchivo{}
			disco_actual.Seek(int64(byteToInt(superB.S_block_start[:])), 0)

			for j := 0; j <= int(inodo.I_block[i]); j++ {
				// --------Se extrae el Bloque Archivo USERS.TXT---------
				var basize int = int(binary.Size(archivo))
				data := leerEnFILE(disco_actual, basize)
				buff := bytes.NewBuffer(data)
				err = binary.Read(buff, binary.BigEndian, &archivo)
				if err != nil {
					Consola += "Binary.Read failed\n"
					msg_error(err)
				}
			}

			//Concatenar el contenido de cada bloque perteneciente al archivo users.txt
			contenidoFile += byteToStr(archivo.B_content[:])
		}
	}

	disco_actual.Close()

	var arregloU_G []string = strings.Split(contenidoFile, "\n")

	for _, filaU_G := range arregloU_G {

		// Se verifica que la fila obtenida del contenido no venga vacia
		if filaU_G != "" {
			var data []string = strings.Split(filaU_G, ",")

			// Verificar ID que no se un U/G eliminado
			if strings.Compare(data[0], "0") != 0 {

				// Verificar que sea tipo Grupo
				if strings.Compare(data[1], "G") == 0 {
					group := data[2]
					if strings.Compare(group, grp_name) == 0 {
						idG, _ := strconv.Atoi(data[0])
						return idG
					}

				}
			}
		}
	}

	return -1
}
