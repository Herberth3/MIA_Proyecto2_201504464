...
func EjecutarRMUSR(usr_name string) {
	Consola = ""
	...
	if IsLoginFlag {
		...
		if CurrentSession.Id_user == 1 && CurrentSession.Id_grp == 1 {
			...
			// Metodo buscarUsuario() pertenece al comando MKUSR
			exist_usr := buscarUsuario(usr_name)
			if exist_usr {
				eliminarUsuario(usr_name)

			} else {
				Consola += "Error: El usuario no existe!\n"
			}
 ...
 * @param string usr_name: nombre del usuario a eliminar
 */
func eliminarUsuario(usr_name string) {
			...
			for j := 0; j < 63; j++ {
				...
				if actual == "\n" {
					if tipo == "U" {
						...
						if strings.Compare(usuario, usr_name) == 0 {
							...
							Consola += "Usuario eliminado con exito!\n"
							flag = true
							break
