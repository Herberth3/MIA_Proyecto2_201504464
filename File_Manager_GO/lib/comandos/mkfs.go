...
func getDatosID(id string, path *string, part_startPartition *int, part_sizePartition *int, error_ *int) {
	...
	for i := 0; i < 4; i++ {
		...
		if strings.Compare(byteToStr(mbr_auxiliar.Mbr_partition[i].Part_name[:]), nombreParticion) == 0 {
			sizePartition = byteToInt(mbr_auxiliar.Mbr_partition[i].Part_size[:])
			startPartition = byteToInt(mbr_auxiliar.Mbr_partition[i].Part_start[:])
		}
	}
...
func formatoEXT2(part_start int, part_size int, path string) {
	...
	disco_actual.Seek(int64(byteToInt(sb.S_inode_start[:])), 0)
	sInodoC := &inodoC
	...
	disco_actual.Seek(int64(byteToInt(sb.S_block_start[:])), 0)
	sBloqueC := &bloqueC
	...
	disco_actual.Seek(int64(byteToInt(sb.S_inode_start[:])+int(i_size)), 0)
	sInodoA := &inodoA
	...
	disco_actual.Seek(int64(byteToInt(sb.S_block_start[:])+int(bc_size)), 0)
	sBloqueA := &bloqueA
